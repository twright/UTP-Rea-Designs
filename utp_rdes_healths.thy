...
lemma R2_st_ex: "R2 (\<Sqinter> s. P\<lbrakk>\<guillemotleft>s\<guillemotright>/st\<^sup><\<rbrakk>) = (\<Sqinter> s. R2(P\<lbrakk>\<guillemotleft>s\<guillemotright>/st\<^sup><\<rbrakk>))"
  by pred_auto
...
lemma R1_R2c_ex_st: "R1 (R2c (\<Sqinter> s. Q\<^sub>1\<lbrakk>\<guillemotleft>s\<guillemotright>/st\<^sup><\<rbrakk>)) = (\<Sqinter> s. R1 (R2c Q\<^sub>1\<lbrakk>s/st\<^sup><\<rbrakk>))"
  by pred_auto

subsection \<open> Identities \<close>

text \<open> We define two identities from reactive designs, which correspond to the regular and 
...
skip_srea_def [pred]: "II\<^sub>R = ((\<Sqinter> s. II\<^sub>C\<lbrakk>\<guillemotleft>s\<guillemotright>/st\<^sup><\<rbrakk>) \<triangleleft> wait\<^sup>< \<triangleright> II\<^sub>C)"

lemma skip_rea_R1_lemma: "II\<^sub>C = (ok\<^sup>< \<longrightarrow>\<^sub>r II)"
  by pred_auto

lemma skip_rea_form: "II\<^sub>C = (II \<triangleleft> ok\<^sup>< \<triangleright> true\<^sub>r)"
  by pred_auto

lemma skip_srea_form: "II\<^sub>R = ((\<Sqinter> s. II\<lbrakk>\<guillemotleft>s\<guillemotright>/st\<^sup><\<rbrakk>) \<triangleleft> wait\<^sup>< \<triangleright> II) \<triangleleft> ok\<^sup>< \<triangleright> true\<^sub>r"
  by pred_auto
...
lemma RD1_Monotonic: "Monotonic RD1"
  unfolding mono_def by pred_auto

lemma RD1_Continuous: "Continuous RD1"
  by pred_auto

lemma R1_true_RD1_closed [closure]: "true\<^sub>r is RD1"
  by pred_auto
...
lemma RD1_R1_cases: "RD1(R1(P)) = (R1(P) \<triangleleft> ok\<^sup>< \<triangleright> true\<^sub>r)"
  by pred_auto
...
(* We can now just prove this by pred_auto and metis; not quite as informative though *)
...
proof -
  have "P = (II\<^sub>C ;; P)"
    by (simp add: assms(3))
  also have "... = ((ok\<^sup>< \<longrightarrow>\<^sub>r II) ;; P)"
    by (simp add: skip_rea_R1_lemma)
  also have "... = (((\<not> ok\<^sup>< \<and> true\<^sub>r) ;; P) \<or> P)"
    sledgehammer
...
definition R3h :: "('s, 't::trace, '\<alpha>) hrel_rsp \<Rightarrow> ('s, 't, '\<alpha>) hrel_rsp" where
 R3h_def [pred]: "R3h(P) = ((\<Sqinter> s. II\<^sub>C\<lbrakk>\<guillemotleft>s\<guillemotright>/st\<^sup><\<rbrakk>) \<triangleleft> wait\<^sup>< \<triangleright> P)"
...
lemma R3c_semir_form:
  ...
  apply(safe)
                      apply simp_all
  (* TODO: find a more automated proof *)
  apply blast
  apply blast
  apply blast
  apply blast
  ...
  (* by (pred_simp, safe, auto intro: order_trans) *)

lemma R3h_semir_form:
  "(R3h(P) ;; R3h(R1(Q))) = R3h(P ;; R3h(R1(Q)))"
  apply(pred_simp, safe)
                      apply(simp_all)
apply blast
  apply blast
  apply blast
...
lemma cond_var_split:
  ...
  by (pred_simp, (metis (full_types) vwb_lens.put_eq)+)
...
proof -
  have "(P ;; Q) = ((P ;; Q)\<lbrakk>True/wait\<^sup><\<rbrakk> \<triangleleft> wait\<^sup>< \<triangleright> (P ;; Q))"
    by (metis cond_var_split cond_var_subst_right wait_vwb_lens SEXP_def aext_var fst_vwb_lens ns_alpha_vwb)
  also have "... = (((II \<triangleleft> wait\<^sup>< \<triangleright> P) ;; Q)\<lbrakk>True/wait\<^sup><\<rbrakk> \<triangleleft> wait\<^sup>< \<triangleright> (P ;; Q))"
    by (metis Healthy_if R3_def SEXP_def assms(1))
  ...
  also have "... = (((II \<and> wait\<^sup>>) ;; II\<^sub>C) \<triangleleft> wait\<^sup>< \<triangleright> (P ;; Q))"
    by (simp add: aext_var seqr_left_one_point_true)
...
lemma R3c_cases: "R3c(P) = ((II \<triangleleft> ok\<^sup>< \<triangleright> R1(true)) \<triangleleft> wait\<^sup>< \<triangleright> P)"
  by (pred_simp; metis (full_types) order_refl)

lemma R3h_cases: "R3h(P) = (((\<Sqinter> s. II\<lbrakk>s/st\<^sup><\<rbrakk>) \<triangleleft> ok\<^sup>< \<triangleright> R1(true)) \<triangleleft> wait\<^sup>< \<triangleright> P)"
...
lemma R3c_subst_wait: "R3c(P) = R3c(P \<^sub>f)"
  by (simp add: R3c_def aext_var cond_var_subst_right)

lemma R3h_subst_wait: "R3h(P) = R3h(P \<^sub>f)"
  by (metis R3c_subst_wait R3h_cancels_R3c)
...
lemma RHS_inf: "\<^bold>R\<^sub>s(P \<sqinter> Q) = \<^bold>R\<^sub>s(P) \<sqinter> \<^bold>R\<^sub>s(Q)"
  using Continuous_choice_dist RHS_Continuous by blast
...
lemma SRD_RHS_H1_H2: "SRD(P) = \<^bold>R\<^sub>s(\<^bold>H(P))"
  ...
  apply safe
  apply (auto)
  apply (metis order_refl trace_class.diff_cancel)+
  done
...
lemma SRD_ok_true_wait_true [usubst]:
  assumes "P is SRD"
  shows "P\<lbrakk>True,True/ok\<^sup><,wait\<^sup><\<rbrakk> = (\<Sqinter> s. II\<lbrakk>s/st\<^sup><\<rbrakk>)\<lbrakk>True,True/ok\<^sup><,wait\<^sup><\<rbrakk>"
proof -
  have "P = (\<Sqinter> s. II\<lbrakk>s/st\<^sup><\<rbrakk>) \<triangleleft> ok\<^sup>< \<triangleright> R1 true \<triangleleft> wait\<^sup>< \<triangleright> P"
    by (metis Healthy_def R3h_cases SRD_healths(3) assms)
  moreover have "((\<Sqinter> s. II\<lbrakk>s/st\<^sup><\<rbrakk>) \<triangleleft> ok\<^sup>< \<triangleright> R1 true \<triangleleft> wait\<^sup>< \<triangleright> P)\<lbrakk>True,True/ok\<^sup><,wait\<^sup><\<rbrakk> = (\<Sqinter> s. II\<lbrakk>s/st\<^sup><\<rbrakk>)\<lbrakk>True,True/ok\<^sup><,wait\<^sup><\<rbrakk>"
    by pred_auto
...
lemma SRD_left_zero_2:
  assumes "P is SRD"
  shows "(\<Sqinter> s. II\<lbrakk>s/st\<^sup><\<rbrakk>)\<lbrakk>True,True/ok\<^sup><,wait\<^sup><\<rbrakk> ;; P = (\<Sqinter> s. II\<lbrakk>s/st\<^sup><\<rbrakk>)\<lbrakk>True,True/ok\<^sup><,wait\<^sup><\<rbrakk>"
proof -
  have "(\<Sqinter> s. II\<lbrakk>s/st\<^sup><\<rbrakk>)\<lbrakk>True,True/ok\<^sup><,wait\<^sup><\<rbrakk> ;; R3h(P) = (\<Sqinter> s. II\<lbrakk>s/st\<^sup><\<rbrakk>)\<lbrakk>True,True/ok\<^sup><,wait\<^sup><\<rbrakk>"
